...
\begin{document}
	...
	In our model, direct computation of the posterior densities for the latent variables $\boldsymbol{b}$ and $\boldsymbol{\eta}$---i.e., $P(\boldsymbol{b}|\,\boldsymbol{x},\boldsymbol{a}, \boldsymbol{r},\boldsymbol{t})$ and $P(\boldsymbol{\eta}|\,\boldsymbol{y},\boldsymbol{a}, \boldsymbol{r},\boldsymbol{t})$---is not possible. However, it is possible to augment the data by candidate receivers $\boldsymbol{u}$ such that we can obtain their conditional posterior by conditioning on samples of $\boldsymbol{u}$. %from $P(\boldsymbol{b}, \boldsymbol{u}|\,\boldsymbol{x},\boldsymbol{a}, \boldsymbol{r},\boldsymbol{t})$ and $P(\boldsymbol{\eta}, \boldsymbol{u}|\, \boldsymbol{y},\boldsymbol{a}, \boldsymbol{r},\boldsymbol{t})$.
	...
	\begin{itemize}
		...
		\item[2.] $\mbox{gender}\_\,\mbox{sender}(a)$: ${x}_{adr2} = I(\mbox{gender of sender } a= \mbox{female});$
		\item[3.] $\mbox{gender}\_\,\mbox{receiver}(r)$: ${x}_{adr3} = I(\mbox{gender of receiver } r= \mbox{female});$
		\item[4.] $\mbox{gender}\_\,\mbox{homophily}(a, r)$: ${x}_{adr4} = I({x}_{adr2}={x}_{adr3});$ 	   		
		\item[5.] $\mbox{outdegree}(a)$: ${x}_{adr5} =\sum_{d^\prime: t_{d^\prime} \in l_d} I(a_{d^\prime} = a)$;
		\item[6.] $\mbox{indegree}(r)$: ${x}_{adr6}=\sum_{d^\prime: t_{d^\prime} \in l_d} I(u_{d^\prime r} = 1)$;
		\item[7.] $\mbox{hyperedge}\_\,\mbox{size}(a)$: ${x}_{adr7}=\sum_{d^\prime: t_{d^\prime} \in l_d} \sum_{r=1}^A I(a_{d^\prime} = a)\,I(u_{d^\prime r} = 1)$;
		\item[8.] $\mbox{interaction}(a)$: ${x}_{adr8} = {x}_{adr5}\times{x}_{adr7};$
		\item[9.] $\mbox{send}(a, r)$: ${x}_{adr9}=\sum_{d^\prime: t_{d^\prime} \in l_d} I(a_{d^\prime} = a)\,I(u_{d^\prime r} = 1)$;
		\item[10.] $\mbox{receive}(a, r)$: ${x}_{adr10}=\mbox{send}(r,a)$;
		\item[11.] $\mbox{two}\_\,\mbox{send}(a, r)$: ${x}_{adr11} = \sum_{h \neq a, r} \mbox{send}(a,h)\,\mbox{send}(h,r)$;
		\item[12.] $\mbox{two}\_\,\mbox{receive}(a, r)$: ${x}_{adr12}= \sum_{h \neq a, r} \mbox{send}(h,a)\,\mbox{send}(r,h)$;
		\item[13.] $\mbox{sibling}(a, r)$: ${x}_{adr13}=\sum_{h \neq a, r} \mbox{send}(h,a)\,\mbox{send}(h,r)$;
		\item[14.] $\mbox{cosibling}(a, r)$: ${x}_{adr14}=\sum_{h \neq a, r} \mbox{send}(a,h)\,\mbox{send}(r,h)$;
	\end{itemize}
	where $I(\cdot)$ is an indicator function. The network statistics (5--14) are designed so that their coefficients have a straightforward interpretations. The function ``outdegree$(a)$" and ``indegree$(r)$" measure the gregariousness and popularity effects of the node by counting the number of emails sent from $a$ and received by $r$, respectively, within the last 7 days. The gregariousness effect refers to the tendency for nodes that sent many edges in the past to continue to do so in the future. The popularity effect refers to the tendency for nodes that received many edges in the past to continue to do so in the future. Moreover, in order to capture the individual tendency of sent emails to two or more receivers, we include the statistic ``$\mbox{hyperedge}\_\,\mbox{size}(a)$"---the number of emails sent from $a$ within last 7 days where emails with $k$ number of receivers are counted as $k$ separate emails---as a variant of outdegree statistic, accounting for hyperedges. We also include the interaction term, ``$\mbox{interaction}(a)$", between outdegree and hyperedge size. This interaction allows us to model a possible tradeoff between the hyperedge size and the total number of edges sent. Dyadic statistics ``send$(a, r)$" and ``receive$(a, r)$" are defined as above such that these covariates measure the number of emails sent from $a$ to $r$ and $r$ to $a$, respectively, within the last 7 days. In the example of triadic statistics, the covariate ``$\mbox{two}\_\,\mbox{send}(a, r)$" counts the pairs of emails involving some node $h$ distinct from $a$ and $r$ such that emails from $a$ to $h$ and $h$ to $r$ are both observed within the last 7 days. This statistic captures the tendency for emails to close transitive triads (i.e., triads in which $a$ sends to $r$ and $h$, and $r$ sends to $h$). We include other triadic covariates that behave similarly and exhibit analogous interpretations, which are illustrated in Figure \ref{figure:netstats}.
	\begin{figure}[!t]
		\centering
...
For the event timing features $\boldsymbol{y}$, introduced in Section \ref{subsec:Time}, we identify a set of covariates which may affect ``time to the next email." Similar to the edge covariates, we include nodal statistics which are time-invariant (such as gender or manager status) or time-dependent (such as the network statistics used for $\boldsymbol{x}$). In addition, we select some edge-specific covariates based on the temporal aspect of the $(d-1)^{\textrm{th}}$ email---e.g., whether the previous email was sent (1) during the weekend and (2) before or past midday (AM/PM)---since we expect the email interactions within county government to be less active during the weekend and in the evening. To be specific, the timestamp statistics are defined as
	\begin{itemize}
		\item[1.] intercept: ${y}_{ad1} =1$;
		\item[2.] $\mbox{gender}(a)$: ${y}_{ad2}=I(\mbox{gender of sender }a= \mbox{female})$;
		\item[3.] $\mbox{manager}(a)$: ${y}_{ad3}=I(\mbox{sender }a \mbox{ is the County Manager})$;
		\item[4.] $\mbox{outdegree}(a)$: ${y}_{ad4} =\sum_{d^\prime: t_{d^\prime} \in l_d} I(a_{d^\prime} = a)$;
		\item[5.] $\mbox{indegree}(a)$: ${y}_{ad5}=\sum_{d^\prime: t_{d^\prime} \in l_d} I(u_{d^\prime a} = 1)$;
		\item[6.] $\mbox{weekend}(d)$: ${y}_{ad6} = I(t_{d-1} \mbox{ is during the } \mbox{weekend})$;
		\item[7.] $\mbox{PM}(d)$: ${y}_{ad7}= I(t_{d-1} \mbox{ in } \mbox{PM})$.
	\end{itemize}
	...
	\begin{algorithm}[!t]
		...
		\begin{algorithmic}
			...
			\IF{$a_d=$  NA}
			\STATE compute the vector $\boldsymbol{\pi}_d$, where $\pi_{da}=f_{\tau}(\tau_{d}; \mu_{ad}, V(\mu_{ad}))\times \prod_{a^\prime\neq a}\big(1-F_{\tau}(\tau_{d}; \mu_{a^\prime d}, V(\mu_{a^\prime d})) \big)$ 
			\STATE draw $a_d \sim \mbox{Categorical}(\boldsymbol{\pi}_d)$ 
			\ENDIF
			\FOR{$r\in [A]_{\backslash a_d}$}
			\IF{$r_{dr}=$ NA}
			\STATE draw $r_{dr} \sim \mbox{Bernoulli}(p_{dr}),$
			where $p_{dr}=\frac{\exp(\lambda_{adr})}{\exp(\lambda_{adr})+\text{I}(\lVert\boldsymbol{u}_{ad\backslash r}\rVert_1 > 0 )}$ %as defined in equation (\ref{eqn:latentreceiver})
			\ENDIF
			\ENDFOR
			\IF{$t_d=$ NA}
			\STATE draw $\boldsymbol{\tau}^{\textrm{(new)}}_d \sim f_{\tau}(\tau_{d}; \mu_{a_d d}, V(\mu_{a_d d}))\times \prod_{a\neq a_d}\big(1-F_{\tau}(\tau_{d}; \mu_{a d}, V(\mu_{a_d d})) \big),$ %from equation (\ref{eqn:latenttime}) (without the product term) 
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
with TP denoting true positive (i.e., $\boldsymbol{r}^{\textrm{(obs)}}_{dr}=\boldsymbol{r}^{\textrm{(pred)}}_{dr}=1$), FN denoting false negative (i.e., $\boldsymbol{r}^{\textrm{(obs)}}_{dr}=1$ but $\boldsymbol{r}^{\textrm{(pred)}}_{dr}=0$), and FP denoting false positive (i.e., $\boldsymbol{r}^{\textrm{(obs)}}_{dr}=0$ but $\boldsymbol{r}^{\textrm{(pred)}}_{dr}=1$). Although the generative process for edges (Section \ref{subsec: Tie}) is not directly affected by the choice of timestamp distribution, Figure \ref{figure:PPEresults} (b) reveals slight difference between log-normal and exponential in their performance in predicting missing receiver indicators, where log-normal on average outperforms exponential. Finally, the prediction error for $d^{\textrm{th}}$ missing timestamp is estimated using the median of absolute relative errors, often referred to as  median absolute percentage error (MdAPE), across $N=500$ predictions:
	\begin{figure}[!t]
		\centering
		\begin{tabular}[t]{ccc}
	...
	\end{figure}		
	\begin{equation}
		\mbox{MdAPE}_{d} = \mbox{median}\Big(\Big\{\abs*{\frac{\tau^{\textrm{(obs)}}_d - \tau^{\textrm{(pred)}_1}_{d}}{\tau^{\textrm{(obs)}}_d}},\ldots, \abs*{\frac{\tau^{\textrm{(obs)}}_d - \tau^{\textrm{(pred)}_N}_{d}}{\tau^{\textrm{(obs)}}_d}}\Big\}\Big).
	\end{equation}
	...
	\begin{algorithm}[!t]
		...
		\begin{algorithmic}
			...
			\ENDFOR
			\STATE Store every $r^{\textrm{th}}$ new data $\{(a_{d}, \boldsymbol{r}_{d}, t_{d})\}_{d=1}^D$
			\ENDFOR
		\end{algorithmic}
		\label{alg:PPC}
	\end{algorithm}     \fi  
