...
In what follows, we introduce the continuous-time multicast network model by describing how we assume the generatve process of a time-stamped network data (Section 2), and deriving the sampling equations for Bayesian inference (Section 3), and apply the model to the Montgomery county government email data (Section 4). Finally, the paper finishes in Section 5 with conclusion and discussion.
