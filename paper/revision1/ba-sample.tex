...
The models discussed above are those, like HEM,  in which edge formation is driven by subgraph structure. There is another class of network models, originating with the latent space model introduced by \cite{hoff2002}, in which tie formation is modeled by embedding nodes in a space of latent attributes. Under this modeling framework, each node is attributed with a coordinate in a metric space, and the likelihood of a tie between two nodes is inversely related to the distance between the two nodes in the metric space. The latent space model has seen several extensions, the latest of which tis the latent factor model proposed by \cite{minhas2016inferential}. A number of dynamic extensions of the latent space framework have been developed \citep[e.g., ][]{sarkar2005dynamic,sewell2015latent,ward2013gravity}. Another popular form of latent variable model for networks is the stochastic block model (SBM) \citep{holland1983stochastic,airoldi2008mixed}. In the SBM, each vertex is associated with one or more blocks, and each block is attributed with a vector of parameters that determines the rate at which members of the respective block form ties with members of each block. The blocks reduce vertex-to-vertex tie probabilities to block-to-block tie probabilities. Like the latent space model, the SBM has been extended to model discrete-time longitudinal networks  \citep{matias2017statistical}. The HEM, as we have presented it, does not involve latent node attributes, but in future work the HEM could be extended to incorporate latent node features---resulting in a latent space model and/or SBM for dynamic hyperedges.
	...
	Unlike the candidate receivers above, the conditional posterior density of $\boldsymbol{b}$ does not have a closed form; however $\boldsymbol{b}$ may instead be re-sampled using the Metropolis--Hastings (MH) algorithm. Assuming a generic Normal prior $\boldsymbol{b}\sim N(\boldsymbol{\mu}_b, \Sigma_b)$, the conditional posterior distribution of $\boldsymbol{b}$ is proportional to~\looseness=-1
	\begin{equation}
	...
	Likewise, we use the MH algorithm to update the latent variable $\boldsymbol{\eta}$. Assuming a Normal prior $\boldsymbol{\eta}\sim N(\boldsymbol{\mu}_\eta = \boldsymbol{0}, \Sigma_\eta = 2\times I_Q)$, the conditional posterior distribution of $\boldsymbol{\eta}$ for an untied event case is proportional to~\looseness=-1
	\begin{equation}
...
The sampler uses a blocked Gibbs sampler for the $u_{iej}$. Given the constraint that $\sum_j u_{iej} > 0$, this sampler may be rather inefficient in the case where there are many one-to-one interactions (as is the case in the application, where 83\% of the interactions are dyadic). In
order to go from the state $u_{ie} = (1, 0, 0, 0, 0, 0)$ to $u_{ie} = (0, 1, 0, 0, 0, 0)$ one needs to go through a state where two receivers are activated, which has low probability in this case. It would be good to comment on this, and in general on the mixing of the MCMC sampler. 
...
There are many MH routines in the literature. Which type of MH do you consider? What about the acceptance rate? Is there any smart proposal in this case which helps in increasing the acceptance rate.
...
For the event timing features $\boldsymbol{z}$, introduced in Section \ref{subsec:Time}, we identify a set of covariates which may affect the time until the next event. Similar to the receiver selection features, we include nodal statistics which are time-invariant (such as gender or manager status) or time-dependent (such as the network statistics used for $\boldsymbol{x}$). In addition, we select some event-specific covariates based on the temporal aspect of the $(e-1)^{\textrm{th}}$ event---e.g., whether the previous email was sent (1) during the weekend and (2) before or past midday (AM/PM)---since we expect the email interactions within county government to be less active during the weekend and in the evening. To be specific, the timestamp statistics are defined as
	...
	We evaluated the models' ability to predict out-of-sample events and timestamps on Montgomery County email data. We generated a train--test split of the data by randomly selecting 10\% of senders, receivers, and timestamp variables to be held out. Our model then imputed these missing variables during inference, sampling them from their conditional posterior along with the other latent variables. One important point is that our covariates $\boldsymbol{x}$ and $\boldsymbol{z}$
depend on the observed interaction data in the last 7 days, as written in Section \ref{subsec:Covariates_email}, hence the observations $ \{ (s_e, \boldsymbol{r}_e, t_e)\}_{e=1}^E$ are not conditionally independent given the model parameters. If the sender of event $e$ is missing, we draw from the categorical distribution using the sender probability vector $\boldsymbol{\pi}_e = (\pi_{1e},\ldots,\pi_{Ve})$. As a result, the conditional distribution for the missing observations $s_e$, $r_{ej}$, and $t_e$ should depend on $ \{ (s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime})\}_{e^\prime \in m_e}$, where $m_e = (t_e, t_e+\mbox{7 days}]$. In particular, if the sender $s_e$ is missing, then we draw from the categorical distribution using the sender probability $\boldsymbol{\pi}_e = (\pi_{1e},\ldots,\pi_{Ve})$, where $\pi_{ie}$ follows
		\begin{equation}
		\begin{aligned}
\pi_{ie}=\Big(f_{\tau}(\tau_{ e}; \mu_{i e}, \sigma_\tau^2)\times \prod_{i^\prime\neq i}\big(1-F_{\tau}(\tau_{ e}; \mu_{i^\prime  e}, \sigma_\tau^2) \big)\Big) \times \prod_{e^\prime:t_{e^\prime} \in m_e} L(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime}|s_{e}=i)
		\end{aligned}
				\label{eqn:missingsender}
		\end{equation}
		with $L(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime}|s_{e}=i)$ denoting the joint likelihood of $(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime})$ conditioned on the covariates $\boldsymbol{x}_{e^\prime}$ and $\boldsymbol{z}_{e^\prime}$ calculated assuming $s_{e}=i$. Here, the term $\prod_{e^\prime:t_{e^\prime} \in m_e} L(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime})$ is proportional to the multiplication of equation (\ref{eqn:latentedge}) and equation (\ref{eqn:latenttime}) (or equation (\ref{eqn:latenttime2})), with the product over $e=1,\ldots,E$ replaced by $\{e^\prime:t_{e^\prime} \in m_e\}$. Similarly, if the $j$th receiver indicator $r_{ej}$ is missing, we sample from Bernoulli distribution where
				\begin{equation}
				\begin{aligned}
			P(r_{ej}=1)&=\exp(\lambda_{iej})\times \prod_{e^\prime:t_{e^\prime} \in m_e} L(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime}|r_{ej}=1),\\
			P(r_{ej}=0) &= \text{I}(\lVert\boldsymbol{u}_{ie\backslash j}\rVert_1 > 0 )\times \prod_{e^\prime: t_{e^\prime} \in m_e} L(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime}|r_{ej}=0).
				\end{aligned}
								\label{eqn:missingreceiver}
				\end{equation}
	Finally, when the timestamp $t_e$ is missing, we draw the time increment $\tau_e$ via importance sampling using the proposal distribution $g \sim \mbox{halfcauchy}(5)$, where its conditional distribution follows
			\begin{equation}
			\begin{aligned}
P({\tau}_e|\, \cdot)\propto \Big(f_{\tau}(\tau_{e}; \mu_{s_{e} e}, \sigma_\tau^2)\times \prod_{i\neq s_{e}}\big(1-F_{\tau}(\tau_{e}; \mu_{ie}, \sigma_\tau^2)\big)\Big) \times\prod_{e^\prime:t_{e^\prime} \in m_e} L(s_{e^\prime}, \boldsymbol{r}_{e^\prime}, t_{e^\prime}|\tau_{e}).
			\end{aligned}
							\label{eqn:missingtime}
			\end{equation}
	Algorithm \ref{alg:PPE} outlines this procedure. 
	...
	\end{equation}
		\begin{algorithm}[!t]
			...
			\begin{algorithmic}
				...
				\IF{$s_e=$  NA}
				\STATE compute $\boldsymbol{\pi}_e = (\pi_{1e},\ldots,\pi_{Ve})$---equation (\ref{eqn:missingsender})
				\STATE draw $s_e \sim \mbox{Categorical}(\boldsymbol{\pi}_e)$ 
				\ENDIF
				\FOR{$j\in [V]_{\backslash s_e}$}
				\IF{$r_{ej}=$ NA}
				\STATE draw $r_{ej} \sim \mbox{Bernoulli}(p_{ej}),$
				where $p_{ej}=\frac{P(r_{ej}=1)}{P(r_{ej}=1)+P(r_{ej}=0)}$---equation (\ref{eqn:missingreceiver})
				\ENDIF
				\ENDFOR
				\IF{$t_e=$ NA}
				\STATE draw ${\tau}_e$ using importance sampling---equation (\ref{eqn:missingtime})%from equation (\ref{eqn:latenttime}) (without the product term) 
				\ENDIF
